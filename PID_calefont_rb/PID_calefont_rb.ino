...
void loop()
{
  ...
  if(posact != (int)Output) //si me tengo que mover
  {
    imprime2(Output-posact, posact);
    stepper.step((int)Output-posact); //me muevo
    posact = (int)Output; //guardo posición actual
  }
  delay(delayloop);
}
...
void imprime(int sp, int in, int out)
{
  Serial.print("sp:");
  Serial.print(" ");
  Serial.print(sp);
