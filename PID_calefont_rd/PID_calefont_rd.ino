...
#define Spoint 34 //ºC temp agua salida
...
{
  int outmap = (int)map(out, 0, 255, 0, maxLlave);
  //if(posac != outmap) //si me tengo que mover
  if(abs(posac - outmap)>=5)//me muevo solo si son mas de 5 pasos
  {
    imprime2(outmap-posac, posac);
    stepper.step(outmap-posac); //me muevo
    posac += (outmap-posac); //actualizo posición actual
  }
  imprime2(outmap-posac, posac);
  return posac;
}
